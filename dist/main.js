...
class UniversityManagementSystem {
    ...
    addCourse(course) {
        const checkedCourse = this.courses.find((c) => c.name === course.name &&
    ...
    calculateAverageGrade(studentId) {
        ...
        if (gradedRecords.length === 0)
            return 0;
        const sum = gradedRecords.reduce((acc, g) => acc + g.grade, 0);
...
function runDemonstration() {
    ...
    console.log(`⭐ Оцінки ${st_anna.fullName}:`, gradesAnna.map((g) => g.grade));
    ...
    console.log(`⭐ Відмінники ФКН:`, excellentCS.map((s) => s.fullName)); // Очікувано: ['Анна Коваль']
